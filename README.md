...
The pixels get changed starting from the top left pixel 0,0 and run down the first column until 0,200. They then start encoding again at 1,0. On a full white image the effect is easier to spot, but on any standard photo it isnt really visible unless the image is extremely low resolution. The lowest possible alpha applied to a pixel can only be 238 or -17.
